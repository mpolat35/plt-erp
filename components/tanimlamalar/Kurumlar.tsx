...
const STORAGE_KEY = "kurumlar_liste_v1";

const INITIAL_DATA: Kurum[] = [
  { id: "k1", ad: "Sağlık Bakanlığı", kurumTuru: "Kamu Kurumu", aciklama: "Türkiye Cumhuriyeti Sağlık Bakanlığı", aktifMi: true },
  { id: "k2", ad: "Erpa Teknoloji A.Ş.", kurumTuru: "Özel Sektör", aciklama: "Yazılım ve teknoloji şirketi", aktifMi: true },
  { id: "k3", ad: "Türkiye Teknoloji Vakfı", kurumTuru: "Sivil Toplum Kuruluşu", aciklama: "Teknoloji alanında faaliyet gösteren vakıf", aktifMi: true },
  { id: "k4", ad: "Orta Doğu Teknik Üniversitesi", kurumTuru: "Üniversite / Eğitim", aciklama: "Devlet üniversitesi", aktifMi: false },
];

const KURUM_TURU_OPTIONS = [
  { label: "Seçiniz", value: "" },
  { label: "Kamu Kurumu", value: "Kamu Kurumu" },
  { label: "Özel Sektör", value: "Özel Sektör" },
  { label: "Sivil Toplum Kuruluşu", value: "Sivil Toplum Kuruluşu" },
  { label: "Üniversite / Eğitim", value: "Üniversite / Eğitim" },
];

type DurumFilter = "tumu" | "aktif" | "pasif";

export default function Kurumlar() {
  const [items, setItems] = useState<Kurum[]>([]);
  ...
  const [editingItem, setEditingItem] = useState<Kurum | null>(null);
  const [deletingItem, setDeletingItem] = useState<Kurum | null>(null);
  ...
  const saveToStorage = (updated: Kurum[]) => {
    setItems(updated);
  ...
  const openEdit = (item: Kurum) => {
    setEditingItem(item);
    setFormAd(item.ad);
    setFormTur(item.kurumTuru);
  ...
  const handleSave = () => {
    ...
    if (editingItem) {
      saveToStorage(items.map(r => r.id === editingItem.id ? { ...r, ad: formAd, kurumTuru: formTur, aciklama: formAciklama, aktifMi: formAktif } : r));
    } else {
      const newItem: Kurum = {
    ...
    <div className="space-y-4">
              ...
              <div className="absolute left-0 top-full mt-1.5 w-60 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-30 p-3 space-y-3 animate-in fade-in slide-in-from-top-1 duration-150">
                <div>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer group py-0.5">
                    ...
                    <span className="text-sm text-slate-700 dark:text-slate-300">Kurum Adı</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer group py-0.5">
                    <span className={clsx(
                      "w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 transition-colors",
                ...
                <div>
                  ...
                  {(["tumu", "aktif", "pasif"] as DurumFilter[]).map(d => (
                    <label key={d} className="flex items-center gap-2 cursor-pointer group py-0.5">
                      ...
                      <input type="radio" name="kurumlar-durum" value={d} checked={filterDurum === d} onChange={() => setFilterDurum(d)} className="sr-only" />
              ...
              {filteredItems.length === 0 ? (
                <Tr>
                  <Td colSpan={6} align="center" className="py-12">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <p className="text-slate-500 font-medium">Kayıtlı kurum bulunamadı</p>
                    </div>
                  </Td>
                </Tr>
              ) : (
                paginatedItems.map((item, i) => (
                  <Tr key={item.id}>
                    ...
                    <Td>
                      <span className="text-slate-500 dark:text-slate-400 text-xs">{item.kurumTuru || "-"}</span>
                    </Td>
                    <Td>
                      <div className="text-slate-500 max-w-xs truncate" title={item.aciklama}>
                        {item.aciklama || "-"}
                      </div>
                    </Td>
                    <Td align="center">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {paginatedItems.map((item, i) => (
                <div key={item.id} className="px-4 py-3.5 flex flex-col gap-2">
                  ...
                  {item.kurumTuru && (
                    <span className="text-xs text-slate-400 dark:text-slate-500">{item.kurumTuru}</span>
                  )}
                  {item.aciklama && (
      ...
      <Modal open={modalOpen} onClose={closeModal}>
        ...
        <Modal.Content className="space-y-5">
          ...
          <div>
            <Modal.Label>Kurum Türü</Modal.Label>
            <Select
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Kurum hakkında kısa bir açıklama"
            />
          </div>
          <div className="flex items-center gap-3 pt-2">
            ...
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
              {formAktif ? "Bu Kurum Aktif (" : "Bu Kurum Pasif ("}
          ...
          {deletingItem && (
            <>
              <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                <span className="font-semibold text-slate-800 dark:text-slate-200">"{deletingItem.ad}"</span> kurumunu silmek istiyor musunuz?
              </p>
